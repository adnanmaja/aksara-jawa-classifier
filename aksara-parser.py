...
base_model = models.resnet18(num_classes=20)
base_model.load_state_dict(torch.load("aksara_model.pth", map_location=device))
base_model = base_model.to(device)
base_model.eval()

sandhangan_model = models.resnet18(num_classes=20)
sandhangan_model.load_state_dict(torch.load("sandhangan_model.pth", map_location=device))
sandhangan_model = base_model.to(device)
sandhangan_model.eval()

pasangan_model = models.resnet18(num_classes=20)
pasangan_model.load_state_dict(torch.load("pasangan_model.pth", map_location=device))
...
# === CLASSIFY SEGMENT ===
def classify(image_tensor, model, class_names):
    with torch.no_grad():
